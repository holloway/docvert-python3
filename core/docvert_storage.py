...
class storage_memory_based(storage):
    ...
    def get_friendly_name_if_available(self, filename):
        if filename in self.friendly_names:
    ...
    def get_zip_name(self):
        friendly_names = ", ".join(list(self.friendly_names.keys()))
        if friendly_names != "":
    ...
    def __str__(self):
        return '<memory based storage with these keys "%s">' % list(self.storage.keys())
