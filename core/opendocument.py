...
def extract_useful_binaries(archive, archive_files, storage, prefix, xml_string):
    ...
    for archive_path in archive_files:
        ...
        if extension in extensions:
            storage_path = "%s/file%i.%s" % (prefix, index, extension)
            try:
            ...
            index += 1
    return io.StringIO(lxml.etree.tostring(document).decode('utf-8'))
...
def extract_xml(archive, archive_files):
    ...
    for xml_file_to_extract in xml_files_to_extract:
        if xml_file_to_extract in archive_files:
            ...
            xml_document_string = archive.open(xml_file_to_extract).read().decode('utf-8').replace('<?xml version="1.0" encoding="UTF-8"?>', '<?xml version="1.0"?>')
            document = lxml.etree.fromstring(xml_document_string) #parsing as XML to remove any doctype
            xml_string.write(lxml.etree.tostring(document).decode('utf-8'))
